...
:: 8000 (백엔드), 8080 (프론트엔드) 포트를 사용하는 프로세스를 찾아 강제 종료 (오류 방지)
...
:: 프론트엔드 실행 (새 창)
start "Stock Frontend UI (Web) - 이 창을 닫지 마세요" cmd /c "title Stock Frontend UI && echo [프론트엔드 웹 서버 가동 중...] && cd frontend && python -m http.server 8080"
...
echo.
echo 브라우저가 열렸습니다. (만약 안 열리면 주소창에 http://localhost:8080 입력)
